   ...
   const [cropData, setCropData] = useState(null);
    ...
    const getAcrop = async () => {
      ...
      try {
        let result;
        if (type === "crop") {
          result = await getAcropFarmerAPI(id, reqHeader);
        } else if (type === "tool") {
          result = await getAToolFarmerAPI(id, reqHeader);
        }
          ...
          {cropData&& 
            ...
            <div>
              ...
              <p className="text-green-600 text-2xl font-semibold mt-4">
                {cropData?.price} <span className="text-sm text-gray-500">/ {cropData?.unit}</span>
              </p>
...
export default FarmerViewProduct;
